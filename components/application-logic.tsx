...
export const ApplicationLogic: React.FC<ApplicationLogicDemoProps> = ({
  requiredSkills,
  userSkills,
}) => {
  ...
  const hasAllVerifiedSkills = requiredSkills.every((required) => {
    return userSkills.some(
      (user) => user.name === required.name && user.verified,
    );
  });

  const missingSkills = requiredSkills.filter(
    (required) =>
      !userSkills.some((user) => user.name === required.name && user.verified),
  );
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-3xl border-2 border-gray-200">
      ...
      {!hasAllVerifiedSkills && (
        <div className="mb-6 rounded-2xl bg-[#FFE4E4] border-2 border-[#FF4B4B] p-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-[#D33131]">
              Missing Verified Skills
            </p>
            <p className="text-sm text-[#D33131]">
              {missingSkills.map((s) => s.name).join(", ")} must be verified
      ...
      <div className="mt-6 p-4 bg-gray-100 rounded-xl text-xs text-gray-600">
        <p>
          <strong>Has all verified skills:</strong>{" "}
          {hasAllVerifiedSkills ? "Yes" : "No"}
        </p>
        <p>
