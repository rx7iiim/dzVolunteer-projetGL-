...
import { Clock, MapPin, Users, Calendar, Heart } from "lucide-react";
...
export default function FollowingMissionsPage() {
  const [missions, setMissions] = useState<Mission[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchFollowingMissions = async () => {
      try {
        const response = await missionService.getFollowingMissions();
        ...
        if (err.response?.status === 404) {
          setError(
            err.response.data?.detail ||
              "You are not following any organizations with missions.",
          );
        } else {
          setError(
            "Failed to fetch following missions. Please try again later.",
          );
        }
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-lg text-muted-foreground">
          Missions from organizations you follow
        </p>
      </div>
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            <p className="text-muted-foreground">
              You are not following any organizations with active missions.
            </p>
          </CardContent>
        </Card>
      ) : (
