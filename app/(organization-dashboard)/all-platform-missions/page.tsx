...
export default function AllPlatformMissionsPage() {
  // --- State ---
  ...
  const fetchAllMissions = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${APIURL}/api/missions/all-missions/`, {
        headers: {
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">All Platform Missions</h2>
          <p className="text-muted-foreground">
            Browse all missions available on the platform.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border-2 border-dashed rounded-xl bg-slate-50">
          ...
          <p className="text-muted-foreground mb-4">
            There are currently no missions available on the platform.
          </p>
        </div>
      ) : (
        // Missions Grid
