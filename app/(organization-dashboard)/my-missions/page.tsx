...
import { useAuthFetch } from "@/hooks/useAuthFetch"; // 1. Import Hook
import {
  Calendar,
  MapPin,
  Users,
...
// Import the Create Modal Component
...
export default function OrganizationMissionsList() {
  ...
  const fetchMissions = useCallback(async () => {
    try {
      // The hook handles the token and headers automatically
      const data = await authFetch(`${APIURL}/api/missions/`);
      setMissions(data || []);
    } catch (err) {
      // If error is "No missions found...", we treat it as an empty list (success)
      // otherwise, the hook's 'error' state will be used in the UI
      setMissions([]);
    }
  }, [authFetch]);
